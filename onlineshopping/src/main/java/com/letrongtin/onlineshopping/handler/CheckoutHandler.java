package com.letrongtin.onlineshopping.handler;
...
import com.letrongtin.onlineshopping.model.CheckoutModel;
...
import com.letrongtin.shoppingbackend.dao.UserDAO;
...
public class CheckoutHandler {
	...
	public CheckoutModel init(String email) throws Exception{
	...
	public List<Address> getShippingAddresses(CheckoutModel model) {
		...
		List<Address> shippings = userDAO.listShippingAddresses(model.getUser().getId());
		
		if(shippings.size() == 0) {
			shippings = new ArrayList<>();
		}

		shippings.add(shippings.size(), userDAO.getBillingAddress(model.getUser().getId()));			
